...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, reference_label_ids):
...
class MsparsEntityProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
    ...
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            assert len(text.split()) == len(label)
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    label_map_reference['x'] = len(label_list) + 1
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, labels, num_labels, use_one_hot_embeddings, dropout_rate):
    ...
    if is_training:
        output_layer = tf.keras.layers.Dropout(rate=dropout_rate)(output_layer)
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, logits, pred_ids, probabilities) = create_model(
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        ori_labels, des_labels, probabilities, reference = [], [], [], []
        for index, each in enumerate(result):
            if index % 1000 == 0:
                tf.logging.info("Processing example: %d" % index)
            ori_labels.append(each["label_ids"])
            des_labels.append(each["pred_ids"])
            probabilities.append(each['probabilities'])
            reference.append(each['reference_label_ids'])

        with codecs.open(os.path.join(FLAGS.output_dir, 'labeled_results.json'), 'w', encoding='utf8') as f:
            ori_labels = np.array(ori_labels).tolist()
            des_labels = np.array(des_labels).tolist()
            probabilities = np.array(probabilities).tolist()
            reference = np.array(reference).tolist()
            json.dump((ori_labels, des_labels, probabilities, reference), f, indent=2)
